...
for (repl in c(1,2,3)){
  ...
  if (replace_lisdhs[repl] == "LIS"){
    ...
    reference_sample_1 = readBin(con, integer(), n = l*w)
    
    # reshape the bindata
    reference_sample_1 <- array(reference_sample_1, dim=c(l, w))
    reference_sample_1 <-  as.data.frame(reference_sample_1) #was frame
    
    colnames(reference_sample_1) <- c('WEALTH', 'RURAL', 'FARMING', 'AGECAT', 'GENDER', 
                                      'EDUCAT', 'HHTYPE', 'HID', 'RELATE', 'HHSIZECAT', 'WEIGHTS')
    
    reference_sample_1['Replacecountry'] <- repl
    ...
    if (repl > 1){
      maxhid <- max(reference_sample$HID)
      reference_sample_1$HID <- reference_sample_1$HID + maxhid
    }
    
    
    
    reference_sample <- rbind(reference_sample, reference_sample_1)
...
weights1 <- sum(reference_sample[reference_sample$Replacecountry == 1,]$WEIGHTS)
weights2 <- sum(reference_sample[reference_sample$Replacecountry == 2,]$WEIGHTS)
weights3 <- sum(reference_sample[reference_sample$Replacecountry == 3,]$WEIGHTS)

maxweight <- max(weights1, weights2, weights3)


reference_sample$Replacecountry[reference_sample$Replacecountry == 1] <- weights1
reference_sample$Replacecountry[reference_sample$Replacecountry == 2] <- weights2
reference_sample$Replacecountry[reference_sample$Replacecountry == 3] <- weights3

reference_sample$Maxweight <- maxweight

reference_sample$Weightcorrection <- reference_sample$Maxweight / reference_sample$Replacecountry

reference_sample$WEIGHTS <- reference_sample$Weightcorrection * reference_sample$WEIGHTS

reference_sample$WEIGHTS <- round(reference_sample$WEIGHTS)
...
#hhsize
hhsize_tib <- read.csv('IPUMS_hhsize_observedmarginals_sept23.csv', sep = ',', header = TRUE)
hhsize_tib <- hhsize_tib[hhsize_tib$country == countryname,]
hhsize_tib <- hhsize_tib %>% select(-country)

hhsize_tib <- pivot_longer(hhsize_tib, cols = c(X1, X2, X3, X4, X5, X6), names_to = "HHSIZECAT", values_to = "Frequency")
hhsize_tib$HHSIZECAT <- as.integer(as.factor(hhsize_tib$HHSIZECAT))


hhsize_tib <- left_join(hhsize_tib, GDLPUMSmatch %>% select(GDLCODE, GEOLEV1_IPUMS), by = 'GDLCODE')
hhsize_tib <- hhsize_tib %>% group_by(HHSIZECAT, GEOLEV1_IPUMS) %>% summarise(Frequency = sum(Frequency))

hhsize_tib <- as_tibble(hhsize_tib)


#hhtype
hhtype_tib <- read.csv('IPUMS_hhtype_observedmarginals_sept23.csv', sep = ',', header = TRUE)
hhtype_tib <- hhtype_tib[hhtype_tib$country == countryname,]
hhtype_tib <- hhtype_tib %>% select(-country)

hhtype_tib <- pivot_longer(hhtype_tib, cols = c(X1, X2, X3, X4, X5, X6, X7, X8), names_to = "HHTYPE", values_to = "Frequency")
hhtype_tib$HHTYPE <- as.integer(as.factor(hhtype_tib$HHTYPE))

hhtype_tib <- left_join(hhtype_tib, GDLPUMSmatch %>% select(GDLCODE, GEOLEV1_IPUMS), by = 'GDLCODE')
hhtype_tib <- hhtype_tib %>% group_by(HHTYPE, GEOLEV1_IPUMS) %>% summarise(Frequency = sum(Frequency))

hhtype_tib <- as_tibble(hhtype_tib)


# age
age_tib <- read.csv('IPUMS_age_observedmarginals_sept23.csv', sep = ',', header = TRUE)
age_tib <- age_tib[age_tib$country == countryname,]
age_tib <- age_tib %>% select(-country)

age_tib <- pivot_longer(age_tib, cols = c(X1, X2, X3, X4, X5, X6, X7), names_to = "AGECAT", values_to = "Frequency")
age_tib$AGECAT <- as.integer(as.factor(age_tib$AGECAT))

age_tib <- left_join(age_tib, GDLPUMSmatch %>% select(GDLCODE, GEOLEV1_IPUMS), by = 'GDLCODE')
age_tib <- age_tib %>% group_by(AGECAT, GEOLEV1_IPUMS) %>% summarise(Frequency = sum(Frequency))

age_tib <- as_tibble(age_tib)


# gender
sex_tib <- read.csv('IPUMS_gender_observedmarginals_sept23.csv', sep = ',', header = TRUE)
sex_tib <- sex_tib[sex_tib$country == countryname,]
sex_tib <- sex_tib %>% select(-country)

sex_tib <- pivot_longer(sex_tib, cols = c(X1, X2), names_to = "GENDER", values_to = "Frequency")
sex_tib$GENDER <- as.integer(as.factor(sex_tib$GENDER))
sex_tib$GENDER <- sex_tib$GENDER -1

sex_tib_rural <- left_join(sex_tib, GDLPUMSmatch %>% select(GDLCODE, GEOLEV1_IPUMS), by = 'GDLCODE')
sex_tib <- sex_tib_rural %>% group_by(GENDER, GEOLEV1_IPUMS) %>% summarise(Frequency = sum(Frequency))

sex_tib <- as_tibble(sex_tib)


# educat
edu_tib <- read.csv('IPUMS_edu_observedmarginals_sept23.csv', sep = ',', header = TRUE)
edu_tib <- edu_tib[edu_tib$country == countryname,]
edu_tib <- edu_tib %>% select(-country)

edu_tib <- pivot_longer(edu_tib, cols = c(X1, X2, X3, X4, X5), names_to = "EDUCAT", values_to = "Frequency")
edu_tib$EDUCAT <- as.integer(as.factor(edu_tib$EDUCAT))

edu_tib <- left_join(edu_tib, GDLPUMSmatch %>% select(GDLCODE, GEOLEV1_IPUMS), by = 'GDLCODE')
edu_tib <- edu_tib %>% group_by(EDUCAT, GEOLEV1_IPUMS) %>% summarise(Frequency = sum(Frequency))

edu_tib <- as_tibble(edu_tib)
...
sum_indi <- sex_tib_rural %>% group_by(GDLCODE) %>% summarise(Population = sum(Frequency))
...
for (regnr in GEOLEV1regions) {
  ...
  hhtype_tib1reg <- hhtype_tib[hhtype_tib$GEOLEV1_IPUMS == regnr,]
  hhtype_tib1reg <- hhtype_tib1reg %>% select(-GEOLEV1_IPUMS)
  
  hhsize_tib1reg <- hhsize_tib[hhsize_tib$GEOLEV1_IPUMS == regnr,]
  hhsize_tib1reg <- hhsize_tib1reg %>% select(-GEOLEV1_IPUMS)
  
  age_tib1reg <- age_tib[age_tib$GEOLEV1_IPUMS == regnr,]
  age_tib1reg <- age_tib1reg %>% select(-GEOLEV1_IPUMS)
  
  edu_tib1reg <- edu_tib[edu_tib$GEOLEV1_IPUMS == regnr,]
  edu_tib1reg <- edu_tib1reg %>% select(-GEOLEV1_IPUMS)
  
  sex_tib1reg <- sex_tib[sex_tib$GEOLEV1_IPUMS == regnr,]
...
for (regnr in GEOLEV1regions) {
  ...
  name = paste0(isocode, '_DHS_oct23_unknownsurvey_synthpop_', as.character(regnr), '.dat')
...
if ((check_sums_survey - check_sums_synth) <= 0.01){
  ...
  print(sum(df_allfreqs$Synth_freq))
...
filename_dferrors <- paste0("synth_errors_estsurvey_DHS_nov23_", isocode, ".csv")
