...
farm_presence <- 0 #set farming to 0, because it's not included in error calculation. 
...
OG_freq <- function(){
  ...
  beginlist_freq = list('> print((data.frame(xtabs(hpopwgt ~ rural_number + HHTYPE_CAT + region_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                        ...
                        '> print((data.frame(xtabs(hpopwgt ~ rural_number + INCOMEQQ + region_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
  ...
  endlist_freq = list('> print((data.frame(xtabs(hpopwgt ~ rural_number + HHSIZECAT + region_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                      '> print((data.frame(xtabs(hpopwgt ~ rural_number + farming_number + region_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                      '> print((data.frame(xtabs(hpopwgt ~ rural_number + INCOMEQQ + region_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                      '> print((data.frame(xtabs(ppopwgt ~ rural_number + AGECAT + region_number, data = dfmerg))$Freq)/sum(dfmerg$ppopwgt))',
                      '> print((data.frame(xtabs(ppopwgt ~ rural_number + education + region_number, data = dfmerg))$Freq)/sum(dfmerg$ppopwgt))',
  ...
  beginlist_freqtotal = list('> print((data.frame(xtabs(hpopwgt ~ rural_number + HHTYPE_CAT, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                             ...
                             '> print((data.frame(xtabs(hpopwgt ~ rural_number + INCOMEQQ, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
  ...
  endlist_freqtotal = list('> print((data.frame(xtabs(hpopwgt ~ rural_number + HHSIZECAT, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                           '> print((data.frame(xtabs(hpopwgt ~ rural_number + farming_number, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                           '> print((data.frame(xtabs(hpopwgt ~ rural_number + INCOMEQQ, data = dfhead))$Freq)/sum(dfhead$hpopwgt))',
                           '> print((data.frame(xtabs(ppopwgt ~ rural_number + AGECAT, data = dfmerg))$Freq)/sum(dfmerg$ppopwgt))',
                           '> print((data.frame(xtabs(ppopwgt ~ rural_number + education, data = dfmerg))$Freq)/sum(dfmerg$ppopwgt))',
...
if (liscodeletters %in% c('gr', 'be', 'ie', 'cn', 'in', 'it', 'jp')){
  ...
  # when a region is missing in individual_marginals_UrbanRural.csv
...
for (regnr in regionnumbers){
  ...
  if (liscodeletters %in% c('gr', 'be', 'ie', 'cn', 'in', 'it', 'jp')){
    ...
    for (gdlcode in GDL_pop_country_regnr$GDLCODE){
      ...
      name_regGDL <- paste0(isocode, '_nov23_unknownmarginal_synthpop_gdl', as.character(gdlcode), '.dat')
...
read_freqs_LIS_regions_unknown2 <- function(txtpathfreq, beginline, endline, regionnumbers){
...
print('check_sums_survey')
print(check_sums_survey)
print('check_sums_synth')
print(check_sums_synth)
...
filename_dferrors <- paste0("synth_errors_estmarg_LIS_nov23_", isocode, ".csv")
#filename_indiperregion <- paste0("Individuals_est_marg_aug23_", countryname, ".csv")
