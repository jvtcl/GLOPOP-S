...
dhscountries <- read.csv('DHScountries_july23.csv', sep = ';') #was eerst DHScountries.csv
...
for (repl in c(1,2,3)){
  ...
  if (replace_lisdhs[repl] == "LIS"){
    ...
    reference_sample_1 <-  as.data.frame(reference_sample_1)
...
for (regnr in GEOLEV1regions) {
  ...
  ml_fitWEIGHTS <- reference_sample$WEIGHTS #WEIGHTS van reference sample denk ik. 
...
# per regio, percentages wealth groepen uitrekenen. 
...
for (regnr in GEOLEV1regions) {
  ...
  name = paste0(isocode, '_DHS_oct23_unknown2_synthpop_', as.character(regnr), '.dat')
...
filename_dferrors <- paste0("synth_errors_unknown2_DHS_nov23_", isocode, ".csv")

#filename_indiperregion <- paste0("Individuals_per_region_unknown2_DHS_oct23_", countryname, ".csv")
